...
- Переименовывает первый выделенный (или просто первый) трек в **Beat – Instrumental**, добавляет
  ReaEQ, ReaComp и JS Saturation с настройками под обнаруженный тональный баланс.
...
5. **Контроль.** Проверь уровни посылов (`Vox Verb Aux`, `Vox Delay Aux`, `Parallel Crush`), откорректируй, если нужно. Лимитер на мастере уже удерживает пики. Сайдчейн между битом и вокалом по умолчанию выключен — лид не уходит в тишину; если нужен обратный дукинг (вокал управляет бочкой/битом), добавь посыл вручную на нужную шину.
...
Если замечаешь несоответствия (например, хочется иная форма реверба или другая длина рендера), дай обратную связь — параметры легко обновляются в одном месте.
